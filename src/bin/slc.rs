...
fn main() {
    ...
    let input_data = {
        ...
        f.read_to_string(&mut input_data).expect(&format!("Error reading file {}", input_file));
        input_data
    };

    let module = compile_from_source(&input_data).expect("Error while compiling module");
    let output = match format {
        "yaml" => serde_yaml::to_string(&module).expect("Error serializing to YAML").into_bytes(),
        "bincode" => bincode::serialize(&module).expect("Error serializing to Bincode"),
        _ => panic!(format!("Invalid format: {}", format))
    };
    let mut outfile = File::create(&output_file).expect(&format!("Couldn't create file {}", output_file));
    outfile.write_all(&output).expect(&format!("Error while writing to {}", output_file));
}

...
pub fn default_builtins() -> Builtins {
  Builtins::new()
    .with_builtin(Builtin::binary("std", "+", |a, b| {
    ...
    .with_builtin(Builtin::contextual("std", "list", None, |ctx, args| {
    ...
    .with_builtin(Builtin::contextual("std", "range", Some(2), |ctx, args| {
      let (a, b) = (args[0], args[1]);
      match (&a.value, &b.value) {
    ...
    .with_builtin(Builtin::contextual("std", "map", Some(2), |ctx, args| {
    ...
    .with_builtin(Builtin::contextual("std", "slice", Some(3), |ctx, args| {
    ...
    //   output.
    .with_builtin(Builtin::contextual("rand", "rng", Some(2), |ctx, args| {
...
mod test {
  ...
  fn allocating_builtins_reserve_before_building_results() {
    ...
    let cases = [
      ...
      (
        "(fn main () (fn id (x) x) (std.map (std.list 1 2 3) id))".to_string(),
        2,
        3 * ptr,
      ),
      (
